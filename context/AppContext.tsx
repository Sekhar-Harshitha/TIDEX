...
import { Report, User, ReportStatus, HazardCategory, UserRole, AppNotification, Message, Language, MeshPeer, MeshPacket, MeshLog, MeshPacketType, GeoLocation, ElectricalZone, ConnectivityMode, SafeZone } from '../types';
...
interface AppContextType {
  ...
  toggleShutdown: (zoneId: string, level: 'Active' | 'Shutdown' | 'Partial') => void;
  ...
  verifyOtp: (type: 'email' | 'phone', value: string, code: string) => Promise<void>;
  addReport: (report: Report) => void;
  updateReportStatus: (id: string, status: ReportStatus, reasoning?: string) => void;
...
const SEED_USERS: User[] = [
  { id: 'u1', name: 'Admin Officer', email: 'admin@tidex.com', phone: '0000', strikes: 0, role: UserRole.ADMIN, isBanned: false },
  { id: 'u2', name: 'Ravi Kumar', email: 'ravi@gmail.com', phone: '9876543210', strikes: 0, role: UserRole.CITIZEN, isBanned: false },
  { id: 'u3', name: 'Priya Sharma', email: 'priya@gmail.com', phone: '9988776655', strikes: 2, role: UserRole.CITIZEN, isBanned: false }, 
];
...
const SEED_ZONES: ElectricalZone[] = [
  { id: 'z1', name: 'Marina Zone A', status: 'Active', riskLevel: 'Low', transformers: 12, affectedCustomers: 1500, lastMaintenance: '2026-02-15 09:00' },
  { id: 'z2', name: 'Mylapore Grid', status: 'Active', riskLevel: 'High', transformers: 24, affectedCustomers: 4200, lastMaintenance: '2026-02-10 14:30' },
  { id: 'z3', name: 'Velachery Low', status: 'Shutdown', riskLevel: 'Critical', transformers: 8, affectedCustomers: 950, lastMaintenance: '2026-02-20 11:15' },
  { id: 'z4', name: 'Port Sector', status: 'Active', riskLevel: 'Medium', transformers: 15, affectedCustomers: 3100, lastMaintenance: '2026-01-28 16:45' },
];

const SEED_SAFE_ZONES: SafeZone[] = [
  { id: 'sz1', name: 'Nehru Indoor Stadium', type: 'Shelter', location: { latitude: 13.0850, longitude: 80.2700 }, capacity: 2000, occupancy: 450, status: 'Available', address: 'Periamet, Chennai', phone: '044-25381234' },
  { id: 'sz2', name: 'Apollo Hospital Greams Rd', type: 'Hospital', location: { latitude: 13.0600, longitude: 80.2500 }, capacity: 500, occupancy: 480, status: 'Available', address: 'Greams Road, Chennai', phone: '044-28293333' },
  { id: 'sz3', name: 'Anna University Campus', type: 'High Ground', location: { latitude: 13.0100, longitude: 80.2350 }, capacity: 5000, occupancy: 1200, status: 'Available', address: 'Guindy, Chennai', phone: '044-22357004' },
  { id: 'sz4', name: 'Mylapore Police Station', type: 'Police Station', location: { latitude: 13.0330, longitude: 80.2680 }, capacity: 100, occupancy: 20, status: 'Available', address: 'Kutchery Rd, Mylapore', phone: '044-24981234' },
  { id: 'sz5', name: 'Loyola College Shelter', type: 'Shelter', location: { latitude: 13.0620, longitude: 80.2340 }, capacity: 1500, occupancy: 1500, status: 'Full', address: 'Nungambakkam, Chennai', phone: '044-28178200' },
];
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [token, setToken] = useState<string | null>(localStorage.getItem('tidex_token'));
  ...
  // Location State
  const [deviceLocation, setDeviceLocation] = useState<GeoLocation | null>(null);
  ...
  // Electrical State
  const [electricalZones, setElectricalZones] = useState<ElectricalZone[]>(SEED_ZONES);
  ...
  // Persistence Effects
  useEffect(() => { localStorage.setItem('tidex_reports', JSON.stringify(reports)); }, [reports]);
  useEffect(() => { localStorage.setItem('tidex_users', JSON.stringify(users)); }, [users]);
  useEffect(() => { localStorage.setItem('tidex_messages', JSON.stringify(messages)); }, [messages]);
  useEffect(() => {
    if (currentUser) localStorage.setItem('tidex_currentUser', JSON.stringify(currentUser));
    ...
    if (isMeshActive) {
      interval = setInterval(() => {
        ...
        if (Math.random() > 0.85) {
          ...
          const fakeContent = ["Power out in Sector 4", "Wire down near school", "Need backup at sub-station", "Grid check complete"][Math.floor(Math.random() * 4)];
          const fakePacket = MeshService.createPacket(randomPeer.name, MeshService.BROADCAST_ID, MeshPacketType.TEXT, fakeContent);
  ...
  const handleIncomingPacket = (packet: MeshPacket) => {
    ...
    if (packet.targetId === MeshService.BROADCAST_ID || packet.targetId === currentUser?.id) {
  ...
  const sendMeshMessage = (text: string, targetId: string = MeshService.BROADCAST_ID) => {
    if (!currentUser) return;
    const packet = MeshService.createPacket(currentUser.id, targetId, MeshPacketType.TEXT, text);
    ...
    const myMessage: Message = {
      id: packet.id,
      senderId: 'me',
      receiverId: targetId === MeshService.BROADCAST_ID ? 'Broadcast' : targetId,
      text: text,
      timestamp: Date.now(),
      isRead: true
    };
  ...
  const toggleShutdown = (zoneId: string, level: 'Active' | 'Shutdown' | 'Partial') => {
    setElectricalZones(prev => prev.map(z => z.id === zoneId ? { ...z, status: level } : z));
    
    const zone = electricalZones.find(z => z.id === zoneId);
    if (!zone) return;

    const msg = level === 'Active' 
  ...
  const uploadPhoto = async (photoBase64: string) => {
    try {
      ...
      setCurrentUser(prev => prev ? { ...prev, profile_image_url: data.profile_image_url } : null);
    } catch (err) {
      console.error(err);
      throw err;
    }
  ...
  const verifyOtp = async (type: 'email' | 'phone', value: string, code: string) => {
    try {
      ...
      if (type === 'email') await updateProfile({ ...currentUser!, email: value });
      else await updateProfile({ ...currentUser!, phone: value });
    } catch (err) {
      console.error(err);
      throw err;
    }
  ...
  const addReport = (report: Report) => {
    if (isOnline) {
      ...
      if (isMeshActive) {
        sendMeshMessage(`Hazard Report: ${report.category} at my location.`, MeshService.BROADCAST_ID);
      }
    }
  };
  ...
  const triggerSOS = (location: { lat: number, lng: number }) => {
    ...
    if (isOnline) {
      // ONLINE LOGIC
      console.log("SOS: Online mode detected. Contacting emergency services via API...");
      // Simulate API call to responders
      setTimeout(() => {
        setLatestNotification({
          id: `sos-online-${Date.now()}`,
          title: 'Emergency Services Contacted',
          message: 'First responders have been notified of your location. Stay where you are.',
          timestamp: Date.now()
        });
      }, 1500);
      ...
      // OFFLINE LOGIC
      console.log("SOS: Offline mode detected. Activating Bluetooth Mesh broadcast...");
      ...
      setLatestNotification({
        ...
        message: 'Your distress signal is being relayed across the mesh network to nearby devices.',
        timestamp: Date.now()
      });
    }
  };

  const updateReportStatus = (id: string, status: ReportStatus, reasoning?: string) => {
    setReports(prev => prev.map(r => {
      if (r.id === id) {
        const updatedReport = { ...r, status, aiReasoning: reasoning || r.aiReasoning };
        
        if (status === ReportStatus.VERIFIED || status === ReportStatus.ACTION_TAKEN) {
           const user = users.find(u => u.id === r.userId);
           const msg = status === ReportStatus.ACTION_TAKEN 
      ...
      if (u.id === userId) {
        ...
        if (newStrikes === 2) {
           setLatestNotification({
             ...
             message: 'You have submitted 2 fake reports. One more and your account will be banned.',
             timestamp: Date.now(),
             targetPhoneNumber: u.phoneNumber
           });
        }
        ...
        if (newStrikes >= 3) {
           setLatestNotification({
             ...
             message: 'Your account has been banned due to repeated fake reporting.',
             timestamp: Date.now(),
             targetPhoneNumber: u.phoneNumber
           });
        }
  ...
  const switchUserRole = async (targetRole?: UserRole) => {
    ...
    const newRole = targetRole || (currentUser.role === UserRole.ADMIN ? UserRole.CITIZEN : UserRole.ADMIN);
