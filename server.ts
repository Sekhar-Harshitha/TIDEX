...
app.post('/api/auth/signup', async (req, res) => {
  ...
  try {
    ...
    if (pool) {
      ...
      user = { id: `u-${Date.now()}`, name, email, password: hashedPassword, phone, role: 'CITIZEN', strikes: 0 };
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: user.id, email: user.email }, JWT_SECRET);
    res.json({ 
      user: { id: user.id, name: user.name, email: user.email, role: user.role, phone: user.phone, profile_image_url: user.profile_image_url }, 
      token 
    });
  } catch (err) {
    console.error(err);
...
app.post('/api/otp/send', authenticateToken, async (req: any, res) => {
  ...
  try {
    if (pool) {
      ...
      mockDb.otps.push({ user_id: req.user.id, type, value, code, expires_at: expiresAt, verified: false });
    }
...
app.post('/api/otp/verify', authenticateToken, async (req: any, res) => {
  ...
  try {
    ...
    if (pool) {
      ...
      const otp = mockDb.otps.find(o => o.user_id === req.user.id && o.type === type && o.value === value && o.code === code && o.expires_at > new Date() && !o.verified);
      if (otp) {
        otp.verified = true;
        verified = true;
      }
