...
const StatCard: React.FC<{
  ...
  color: string;
  borderColor: string;
}> = ({ title, value, icon, color, borderColor }) => (
  <div
    className={`bg-midnight-900 border border-midnight-800 rounded-xl p-6 flex items-start justify-between shadow-lg hover:border-${borderColor} transition-all duration-300 group`}
  >
    <div>
      <p className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">
        {title}
      </p>
      <h3 className="text-3xl font-display font-bold text-white mt-1 group-hover:scale-105 transition-transform origin-left">
        {value}
      </h3>
    </div>
    <div
      className={`p-3 rounded-xl ${color} bg-opacity-10 border border-white/5`}
    >
      {icon}
    </div>
  </div>
...
export const Dashboard: React.FC = () => {
  ...
  const fakeReports = reports.filter(
    (r) => r.status === ReportStatus.FAKE,
  ).length;
  ...
  const statusData = [
    ...
    { name: "Pending", value: pendingReports, color: "#f59e0b" },
    {
      ...
      value:
        fakeReports +
        reports.filter((r) => r.status === ReportStatus.SUSPICIOUS).length,
      color: "#ef4444",
    },
  ];
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            Real-time hazard monitoring and analysis.
          </p>
        </div>
            ...
            <div>
              <div className="flex items-end gap-2">
                ...
                <div className="flex flex-col">
                  <span className="text-xs text-ocean-300 font-bold uppercase tracking-tight">
                    {weather.locationName}
                  </span>
                  {weather.isFallback && (
                    <span className="text-[8px] text-amber-400 font-bold uppercase animate-pulse">
                      Fallback Mode
                    </span>
                  )}
                </div>
              ...
              <div className="flex flex-col gap-1">
                <p className="text-xs text-slate-400 flex gap-3">
                  <span className="flex items-center gap-1">
                    <Wind size={10} /> {weather.current.windSpeed}km/h
                  </span>
                  <span className="flex items-center gap-1">
                ...
                {weather.sourceUri && (
                  <a
                    ...
                    className="flex items-center gap-1 text-[9px] text-ocean-400 hover:text-white mt-1 underline"
                  >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          title="Total Reports"
          value={totalReports}
          icon={<Siren className="text-blue-400" />}
          color="bg-blue-400"
          ...
          >
            ...
            <div className="flex-1 bg-midnight-950 rounded-lg border border-midnight-800 relative overflow-hidden">
              ...
              {reports.slice(0, 10).map((report, i) => (
                ...
                >
                  <div
                    className={`w-3 h-3 rounded-full ${report.status === ReportStatus.VERIFIED ? "bg-emerald-500 shadow-[0_0_10px_#10b981]" : "bg-amber-500 shadow-[0_0_10px_#f59e0b]"}`}
                  ></div>
                </div>
              ))}
            ...
            <div className="space-y-3 max-h-[250px] overflow-y-auto pr-2">
              {[...reports]
                ...
                .map((report) => (
                  <div
                    key={report.id}
                    className="flex items-center justify-between p-3.5 bg-midnight-800/50 rounded-lg border border-midnight-700 hover:bg-midnight-800 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`p-2 rounded-lg ${report.status === "Verified" ? "bg-emerald-500/10 text-emerald-400" : "bg-amber-500/10 text-amber-400"}`}
                      >
                        <Radio size={16} />
                      </div>
                      <div>
                        <p className="text-sm text-white font-medium flex items-center gap-2">
                          ...
                          {report.priorityLevel === PriorityLevel.CRITICAL && (
                            <span className="text-[8px] bg-red-500 text-white px-1.5 py-0.5 rounded animate-pulse">
                              CRITICAL
                            </span>
                          )}
                        </p>
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <span className="text-xs font-mono text-slate-500 bg-midnight-950 px-2 py-1 rounded">
                        {report.location.latitude.toFixed(2)},{" "}
