...
class BleMeshManager(private val context: Context) {
    ...
    @SuppressLint("MissingPermission")
    fun advertiseMessage(message: SOSMessage) {
        if (!hasBlePermissions()) {
            onError?.invoke("Missing BLE permissions")
            return
        }
        ...
        val payload = message.toJsonString().toByteArray(StandardCharsets.UTF_8)
        if (payload.size > MAX_ADV_BYTES) {
            onError?.invoke("Payload too large for BLE advertisement (${payload.size} bytes)")
            return
        }
        ...
        advertiseCallback = object : AdvertiseCallback() {
            override fun onStartSuccess(settingsInEffect: AdvertiseSettings) {
                Log.i(TAG, "SOS advertised: id=${message.id} ttl=${message.ttl}")
            }

            override fun onStartFailure(errorCode: Int) {
                onError?.invoke("BLE advertise failed: $errorCode")
            }
        }
    ...
    private fun parseScanResult(result: ScanResult) {
        ...
        val bytes = record.getServiceData(ParcelUuid(SERVICE_UUID)) ?: return
        try {
            val rawJson = String(bytes, StandardCharsets.UTF_8)
            val message = SOSMessage.fromJsonString(rawJson)
            onMessageReceived?.invoke(message)
        } catch (err: Exception) {
            onError?.invoke("Decode error: ${err.message}")
        }
    }
    ...
    companion object {
        ...
        // For legacy advertising payload safety.
        private const val MAX_ADV_BYTES = 160

        val SERVICE_UUID: UUID = UUID.fromString("4d4f5348-1000-4000-8000-000000000001")
    }
}
